...
/// struct for typed errors of method [`get_counterparty`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetCounterpartyError {
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_counterparties`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListCounterpartiesError {
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    UnknownValue(serde_json::Value),
}
...
pub async fn create_counterparty(configuration: &configuration::Configuration, post_counterparty: models::PostCounterparty) -> Result<models::Counterparty, Error<CreateCounterpartyError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_counterparty = post_counterparty;

    let uri_str = format!("{}/api/counterparties", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_counterparty);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Counterparty`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Counterparty`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_counterparty(configuration: &configuration::Configuration, counterparty_guid: &str, include_pii: Option<bool>) -> Result<models::Counterparty, Error<GetCounterpartyError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_counterparty_guid = counterparty_guid;
    let p_query_include_pii = include_pii;

    let uri_str = format!("{}/api/counterparties/{counterparty_guid}", configuration.base_path, counterparty_guid=crate::apis::urlencode(p_path_counterparty_guid));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetCounterpartyError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn list_counterparties(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, r#type: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, guid: Option<&str>, label: Option<&str>, include_pii: Option<bool>) -> Result<models::CounterpartyList, Error<ListCounterpartiesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CounterpartyList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CounterpartyList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListCounterpartiesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
