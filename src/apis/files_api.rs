...
pub async fn create_file(configuration: &configuration::Configuration, post_file: models::PostFile) -> Result<models::PlatformFile, Error<CreateFileError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_file = post_file;
    ...
    req_builder = req_builder.json(&p_body_post_file);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PlatformFile`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PlatformFile`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_file(configuration: &configuration::Configuration, file_guid: &str, include_download_url: Option<&str>) -> Result<models::PlatformFile, Error<GetFileError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_file_guid = file_guid;
    let p_query_include_download_url = include_download_url;

    let uri_str = format!("{}/api/files/{file_guid}", configuration.base_path, file_guid=crate::apis::urlencode(p_path_file_guid));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    if let Some(ref param_value) = p_query_include_download_url {
        req_builder = req_builder.query(&[("include_download_url", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
...
pub async fn list_files(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, r#type: Option<&str>, state: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>) -> Result<models::PlatformFileList, Error<ListFilesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PlatformFileList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PlatformFileList`")))),
        }
    } else {
        let content = resp.text().await?;
