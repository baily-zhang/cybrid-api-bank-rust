...
pub async fn create_bank(configuration: &configuration::Configuration, post_bank: models::PostBank) -> Result<models::Bank, Error<CreateBankError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_bank = post_bank;

    let uri_str = format!("{}/api/banks", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_bank);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Bank`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Bank`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateBankError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a bank.  Required scope: **banks:read**
pub async fn get_bank(configuration: &configuration::Configuration, bank_guid: &str) -> Result<models::Bank, Error<GetBankError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_bank_guid = bank_guid;

    let uri_str = format!("{}/api/banks/{bank_guid}", configuration.base_path, bank_guid=crate::apis::urlencode(p_path_bank_guid));
...
pub async fn list_banks(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, r#type: Option<&str>, guid: Option<&str>) -> Result<models::BankList, Error<ListBanksError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::BankList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::BankList`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_bank(configuration: &configuration::Configuration, bank_guid: &str, patch_bank: models::PatchBank) -> Result<models::Bank, Error<UpdateBankError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_bank_guid = bank_guid;
    let p_body_patch_bank = patch_bank;
    ...
    req_builder = req_builder.json(&p_body_patch_bank);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<UpdateBankError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
