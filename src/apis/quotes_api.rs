...
pub async fn create_quote(configuration: &configuration::Configuration, post_quote: models::PostQuote) -> Result<models::Quote, Error<CreateQuoteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_quote = post_quote;

    let uri_str = format!("{}/api/quotes", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_quote);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateQuoteError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a quote.  Required scope: **quotes:read**
pub async fn get_quote(configuration: &configuration::Configuration, quote_guid: &str) -> Result<models::Quote, Error<GetQuoteError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_quote_guid = quote_guid;

    let uri_str = format!("{}/api/quotes/{quote_guid}", configuration.base_path, quote_guid=crate::apis::urlencode(p_path_quote_guid));
...
pub async fn list_quotes(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, product_type: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, side: Option<&str>) -> Result<models::QuoteList, Error<ListQuotesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::QuoteList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::QuoteList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListQuotesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
