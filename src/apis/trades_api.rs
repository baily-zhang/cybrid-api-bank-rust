...
/// struct for typed errors of method [`create_trade`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateTradeError {
...
pub async fn create_trade(configuration: &configuration::Configuration, post_trade: models::PostTrade) -> Result<models::Trade, Error<CreateTradeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_trade = post_trade;
    ...
    req_builder = req_builder.json(&p_body_post_trade);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Trade`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Trade`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateTradeError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a trade.  Required scope: **trades:read**
pub async fn get_trade(configuration: &configuration::Configuration, trade_guid: &str) -> Result<models::Trade, Error<GetTradeError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_trade_guid = trade_guid;

    let uri_str = format!("{}/api/trades/{trade_guid}", configuration.base_path, trade_guid=crate::apis::urlencode(p_path_trade_guid));
...
pub async fn list_trades(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, account_guid: Option<&str>, state: Option<&str>, side: Option<&str>, label: Option<&str>, created_at_gte: Option<&str>, created_at_lt: Option<&str>, updated_at_gte: Option<&str>, updated_at_lt: Option<&str>) -> Result<models::TradeList, Error<ListTradesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TradeList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TradeList`")))),
        }
    } else {
        let content = resp.text().await?;
