...
/// struct for typed errors of method [`create_deposit_address`]
...
pub async fn create_deposit_address(configuration: &configuration::Configuration, post_deposit_address: models::PostDepositAddress) -> Result<models::DepositAddress, Error<CreateDepositAddressError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_deposit_address = post_deposit_address;

    let uri_str = format!("{}/api/deposit_addresses", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_deposit_address);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DepositAddress`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DepositAddress`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateDepositAddressError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a deposit address.  Required scope: **deposit_addresses:read**
pub async fn get_deposit_address(configuration: &configuration::Configuration, deposit_address_guid: &str) -> Result<models::DepositAddress, Error<GetDepositAddressError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_deposit_address_guid = deposit_address_guid;

    let uri_str = format!("{}/api/deposit_addresses/{deposit_address_guid}", configuration.base_path, deposit_address_guid=crate::apis::urlencode(p_path_deposit_address_guid));
...
pub async fn list_deposit_addresses(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, label: Option<&str>) -> Result<models::DepositAddressList, Error<ListDepositAddressesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::DepositAddressList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::DepositAddressList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListDepositAddressesError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
