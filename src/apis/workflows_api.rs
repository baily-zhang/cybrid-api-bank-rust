...
pub async fn create_workflow(configuration: &configuration::Configuration, post_workflow: models::PostWorkflow) -> Result<models::Workflow, Error<CreateWorkflowError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_workflow = post_workflow;
    ...
    req_builder = req_builder.json(&p_body_post_workflow);
...
pub async fn get_workflow(configuration: &configuration::Configuration, workflow_guid: &str) -> Result<models::WorkflowWithDetails, Error<GetWorkflowError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_workflow_guid = workflow_guid;

    let uri_str = format!("{}/api/workflows/{workflow_guid}", configuration.base_path, workflow_guid=crate::apis::urlencode(p_path_workflow_guid));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WorkflowWithDetails`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WorkflowWithDetails`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_workflows(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>) -> Result<models::WorkflowsList, Error<ListWorkflowsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::WorkflowsList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::WorkflowsList`")))),
        }
    } else {
        let content = resp.text().await?;
