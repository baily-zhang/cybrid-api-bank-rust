...
/// struct for typed errors of method [`create_identity_verification`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreateIdentityVerificationError {
...
/// struct for typed errors of method [`list_identity_verifications`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListIdentityVerificationsError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    UnknownValue(serde_json::Value),
...
pub async fn create_identity_verification(configuration: &configuration::Configuration, post_identity_verification: models::PostIdentityVerification) -> Result<models::IdentityVerification, Error<CreateIdentityVerificationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_identity_verification = post_identity_verification;

    let uri_str = format!("{}/api/identity_verifications", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_identity_verification);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IdentityVerification`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IdentityVerification`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreateIdentityVerificationError> = serde_json::from_str(&content).ok();
...
pub async fn get_identity_verification(configuration: &configuration::Configuration, identity_verification_guid: &str, include_pii: Option<bool>) -> Result<models::IdentityVerificationWithDetails, Error<GetIdentityVerificationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_identity_verification_guid = identity_verification_guid;
    let p_query_include_pii = include_pii;

    let uri_str = format!("{}/api/identity_verifications/{identity_verification_guid}", configuration.base_path, identity_verification_guid=crate::apis::urlencode(p_path_identity_verification_guid));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IdentityVerificationWithDetails`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IdentityVerificationWithDetails`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_identity_verifications(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, counterparty_guid: Option<&str>, state: Option<&str>, r#type: Option<&str>, method: Option<&str>) -> Result<models::IdentityVerificationList, Error<ListIdentityVerificationsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::IdentityVerificationList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::IdentityVerificationList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListIdentityVerificationsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
