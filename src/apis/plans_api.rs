...
pub async fn create_plan(configuration: &configuration::Configuration, post_plan: models::PostPlan) -> Result<models::Plan, Error<CreatePlanError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_plan = post_plan;

    let uri_str = format!("{}/api/plans", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_plan);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Plan`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Plan`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_plan(configuration: &configuration::Configuration, plan_guid: &str) -> Result<models::Plan, Error<GetPlanError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_plan_guid = plan_guid;

    let uri_str = format!("{}/api/plans/{plan_guid}", configuration.base_path, plan_guid=crate::apis::urlencode(p_path_plan_guid));
...
pub async fn list_plans(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, r#type: Option<&str>, state: Option<&str>, source_account_guid: Option<&str>, destination_account_guid: Option<&str>, created_at_gte: Option<&str>, created_at_lt: Option<&str>, updated_at_gte: Option<&str>, updated_at_lt: Option<&str>) -> Result<models::PlanList, Error<ListPlansError>> {
    // add a prefix to parameters to efficiently prevent name collisions
