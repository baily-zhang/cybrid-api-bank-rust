...
pub async fn create_customer(configuration: &configuration::Configuration, post_customer: models::PostCustomer) -> Result<models::Customer, Error<CreateCustomerError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_customer = post_customer;
    ...
    req_builder = req_builder.json(&p_body_post_customer);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Customer`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Customer`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn get_customer(configuration: &configuration::Configuration, customer_guid: &str, include_pii: Option<bool>) -> Result<models::Customer, Error<GetCustomerError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_customer_guid = customer_guid;
    let p_query_include_pii = include_pii;

    let uri_str = format!("{}/api/customers/{customer_guid}", configuration.base_path, customer_guid=crate::apis::urlencode(p_path_customer_guid));
...
pub async fn list_customers(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, r#type: Option<&str>, bank_guid: Option<&str>, guid: Option<&str>, label: Option<&str>, include_pii: Option<bool>) -> Result<models::CustomerList, Error<ListCustomersError>> {
    // add a prefix to parameters to efficiently prevent name collisions
...
pub async fn update_customer(configuration: &configuration::Configuration, customer_guid: &str, patch_customer: models::PatchCustomer) -> Result<models::Customer, Error<UpdateCustomerError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_customer_guid = customer_guid;
    let p_body_patch_customer = patch_customer;
    ...
    req_builder = req_builder.json(&p_body_patch_customer);
