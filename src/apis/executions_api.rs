...
pub async fn create_execution(configuration: &configuration::Configuration, post_execution: models::PostExecution) -> Result<models::Execution, Error<CreateExecutionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_execution = post_execution;

    let uri_str = format!("{}/api/executions", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_execution);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<CreateExecutionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a execution.  Required scope: **executions:read**
pub async fn get_execution(configuration: &configuration::Configuration, execution_guid: &str) -> Result<models::Execution, Error<GetExecutionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_execution_guid = execution_guid;

    let uri_str = format!("{}/api/executions/{execution_guid}", configuration.base_path, execution_guid=crate::apis::urlencode(p_path_execution_guid));
...
pub async fn list_executions(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, plan_guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, r#type: Option<&str>, state: Option<&str>, source_account_guid: Option<&str>, destination_account_guid: Option<&str>, created_at_gte: Option<&str>, created_at_lt: Option<&str>, updated_at_gte: Option<&str>, updated_at_lt: Option<&str>) -> Result<models::ExecutionList, Error<ListExecutionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_page = page;
    let p_query_per_page = per_page;
    let p_query_guid = guid;
    let p_query_plan_guid = plan_guid;
    let p_query_bank_guid = bank_guid;
    let p_query_customer_guid = customer_guid;
    let p_query_type = r#type;
    let p_query_state = state;
    let p_query_source_account_guid = source_account_guid;
    let p_query_destination_account_guid = destination_account_guid;
    ...
    if let Some(ref param_value) = p_query_plan_guid {
        req_builder = req_builder.query(&[("plan_guid", &param_value.to_string())]);
    }
    ...
    if let Some(ref param_value) = p_query_source_account_guid {
        req_builder = req_builder.query(&[("source_account_guid", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_destination_account_guid {
        req_builder = req_builder.query(&[("destination_account_guid", &param_value.to_string())]);
    }
