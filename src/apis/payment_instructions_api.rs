...
/// struct for typed errors of method [`create_payment_instruction`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum CreatePaymentInstructionError {
...
/// struct for typed errors of method [`get_payment_instruction`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetPaymentInstructionError {
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
...
/// struct for typed errors of method [`list_payment_instructions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListPaymentInstructionsError {
    Status400(models::ErrorResponse),
    Status401(models::ErrorResponse),
    Status403(models::ErrorResponse),
    UnknownValue(serde_json::Value),
...
pub async fn create_payment_instruction(configuration: &configuration::Configuration, post_payment_instruction: models::PostPaymentInstruction) -> Result<models::PaymentInstruction, Error<CreatePaymentInstructionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_payment_instruction = post_payment_instruction;

    let uri_str = format!("{}/api/payment_instructions", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_payment_instruction);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PaymentInstruction`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PaymentInstruction`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<CreatePaymentInstructionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

/// Retrieves a payment_instruction.  Required scope: **invoices:read**
pub async fn get_payment_instruction(configuration: &configuration::Configuration, payment_instruction_guid: &str) -> Result<models::PaymentInstruction, Error<GetPaymentInstructionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_payment_instruction_guid = payment_instruction_guid;

    let uri_str = format!("{}/api/payment_instructions/{payment_instruction_guid}", configuration.base_path, payment_instruction_guid=crate::apis::urlencode(p_path_payment_instruction_guid));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetPaymentInstructionError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn list_payment_instructions(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, invoice_guid: Option<&str>) -> Result<models::PaymentInstructionList, Error<ListPaymentInstructionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    let p_query_invoice_guid = invoice_guid;
    ...
    if let Some(ref param_value) = p_query_invoice_guid {
        req_builder = req_builder.query(&[("invoice_guid", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::PaymentInstructionList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::PaymentInstructionList`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<ListPaymentInstructionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
