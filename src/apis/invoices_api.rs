...
pub async fn cancel_invoice(configuration: &configuration::Configuration, invoice_guid: &str) -> Result<models::Invoice, Error<CancelInvoiceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_invoice_guid = invoice_guid;

    let uri_str = format!("{}/api/invoices/{invoice_guid}", configuration.base_path, invoice_guid=crate::apis::urlencode(p_path_invoice_guid));
...
pub async fn create_invoice(configuration: &configuration::Configuration, post_invoice: models::PostInvoice) -> Result<models::Invoice, Error<CreateInvoiceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_invoice = post_invoice;

    let uri_str = format!("{}/api/invoices", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_invoice);
...
pub async fn get_invoice(configuration: &configuration::Configuration, invoice_guid: &str) -> Result<models::Invoice, Error<GetInvoiceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_invoice_guid = invoice_guid;
...
pub async fn list_invoices(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, account_guid: Option<&str>, state: Option<&str>, asset: Option<&str>, environment: Option<&str>, label: Option<&str>) -> Result<models::InvoiceList, Error<ListInvoicesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::InvoiceList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::InvoiceList`")))),
        }
    } else {
        let content = resp.text().await?;
