...
pub async fn create_transfer(configuration: &configuration::Configuration, post_transfer: models::PostTransfer) -> Result<models::Transfer, Error<CreateTransferError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_post_transfer = post_transfer;

    let uri_str = format!("{}/api/transfers", configuration.base_path);
    ...
    req_builder = req_builder.json(&p_body_post_transfer);
...
pub async fn get_transfer(configuration: &configuration::Configuration, transfer_guid: &str) -> Result<models::Transfer, Error<GetTransferError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_transfer_guid = transfer_guid;

    let uri_str = format!("{}/api/transfers/{transfer_guid}", configuration.base_path, transfer_guid=crate::apis::urlencode(p_path_transfer_guid));
...
pub async fn list_transfers(configuration: &configuration::Configuration, page: Option<i32>, per_page: Option<i32>, guid: Option<&str>, transfer_type: Option<&str>, bank_guid: Option<&str>, customer_guid: Option<&str>, account_guid: Option<&str>, state: Option<&str>, side: Option<&str>, label: Option<&str>, txn_hash: Option<&str>, created_at_gte: Option<&str>, created_at_lt: Option<&str>, updated_at_gte: Option<&str>, updated_at_lt: Option<&str>) -> Result<models::TransferList, Error<ListTransfersError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_page = page;
    let p_query_per_page = per_page;
    let p_query_guid = guid;
    let p_query_transfer_type = transfer_type;
    let p_query_bank_guid = bank_guid;
    let p_query_customer_guid = customer_guid;
    let p_query_account_guid = account_guid;
    let p_query_state = state;
    let p_query_side = side;
    let p_query_label = label;
    let p_query_txn_hash = txn_hash;
    let p_query_created_at_gte = created_at_gte;
    let p_query_created_at_lt = created_at_lt;
    let p_query_updated_at_gte = updated_at_gte;
    let p_query_updated_at_lt = updated_at_lt;
    ...
    if let Some(ref param_value) = p_query_transfer_type {
        req_builder = req_builder.query(&[("transfer_type", &param_value.to_string())]);
    }
    ...
    if let Some(ref param_value) = p_query_account_guid {
        req_builder = req_builder.query(&[("account_guid", &param_value.to_string())]);
    }
    ...
    if let Some(ref param_value) = p_query_txn_hash {
        req_builder = req_builder.query(&[("txn_hash", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_created_at_gte {
        req_builder = req_builder.query(&[("created_at_gte", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_created_at_lt {
        req_builder = req_builder.query(&[("created_at_lt", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_updated_at_gte {
        req_builder = req_builder.query(&[("updated_at_gte", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_updated_at_lt {
        req_builder = req_builder.query(&[("updated_at_lt", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::TransferList`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::TransferList`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn update_transfer(configuration: &configuration::Configuration, transfer_guid: &str, patch_transfer: models::PatchTransfer) -> Result<models::Transfer, Error<UpdateTransferError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_transfer_guid = transfer_guid;
    let p_body_patch_transfer = patch_transfer;
    ...
    req_builder = req_builder.json(&p_body_patch_transfer);
